import { _decorator, Component, Slider, Label, ScrollView, Vec4, dynamicAtlasManager, find, Material, UIRenderer, UITransform, Sprite} from 'cc';
const { ccclass, property } = _decorator;

@ccclass('GlowInnerVer2')
export default class GlowInnerVer2 extends Component {
    ...
    private _onSliderChanged() {
        ...
        let realGlowWidthProgress = Math.round(this._glowWidthSlider!.progress * 200);
        ...
        this._updateRenderComponentMaterial({
            glowColor: new Vec4(this._redSlider!.progress,
                this._greenSlider!.progress,
                this._blueSlider!.progress,
                this._alphaSlider!.progress),
    ...
    private _updateRenderComponentMaterial(param: {
        ...
        glowThreshold: number;
            ...
            this._scrollView!.content!.children.forEach((childNode) => {
                let material: Material = childNode.getComponent(UIRenderer)!.getSharedMaterial(0)!;
                material.setProperty("spriteWidth", childNode.getComponent(UITransform)!.contentSize.width);
                material.setProperty("spriteHeight", childNode.getComponent(UITransform)!.contentSize.height);
                ...
                childNode.getComponent(UIRenderer)!.setMaterial(material, 0);

                if (childNode.getComponent(UIRenderer) instanceof Sprite){
